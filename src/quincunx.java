...
public class quincunx extends JPanel implements KeyListener {
  ...
  /*
   * protected void paintMapIso(Graphics g) { int e = 0; for(int
   * c=COLS-1;c>=0;c--) { for(int r=0;r<ROWS;r++) { e =
   * theLevel.getElevation(r*COLS+c); theTilesIso.drawTile(g, e, c, r, e, this);
   * } } int actorX = (int)actors.get(0).getX(); int actorY =
   * (int)actors.get(0).getY(); int actorI = (int)actors.get(0).getI(); int
   * actorE = theLevel.getElevation(actorY*COLS+actorX);
  ...
  protected void paintMapOrtho(Graphics g) {
    ...
    int XX = (int) VPORT.getX();
    int YY = (int) VPORT.getY();
    if (XX < 0) {
      XX = 0;
    } else if ((XX + VPORT.getWidth()) > COLS) {
      XX = COLS - (int) VPORT.getWidth();
    }
    if (YY < 0) {
      YY = 0;
    } else if ((YY + VPORT.getHeight()) > ROWS) {
      YY = ROWS - (int) VPORT.getHeight();
    }
  ...
  boolean canMoveTo(Point src, Point dst) {
    ...
    if (Math.abs(dstElevation - srcElevation) > 1) {
      return (false);
    }

    if (theLevel.collides((int) dst.getX(), (int) dst.getY())) {
      return (false);
    }

    int a = isOccupied(dst);
    if (a > 0) {
      return (!actors.get(a).hasInteraction());
    }
    return (true);
  }
  ...
  int isOccupied(Point t) {
    for (int i = 0; i < actors.size(); i++) {
      if (actors.get(i).getLocation().equals(t)) {
        return (i);
      }
    }
    return (-1);
    ...
    for (r = 0; r < twoPI; r += oneDegree) {
      ...
      if (VPORT.contains(cx, cy) && MPORT.contains(cx, cy)) {
        for (i = 0; i < VRADIUS; i++) {
          ...
          if (theLevel.getElevation((int) cx, (int) cy) == 0)
            break;
          cx += vx;
          cy += vy;
        }
  ...
  int linearize(Point p) {
    return ((int) p.getY() * COLS + (int) p.getX());
  }
